...
module Jekyll
  ...
  class UserIndexPage < Page
    def initialize(site, base, user, entries)
      ...
      # Sanitize titles for all entries so links will work properly
      sanitized_entries = entries.map do |entry|
        entry.merge('sanitized_title' => sanitize_title(entry['title']))
      end
      
      # Pass entries to the page's data
      self.data['user'] = user
      self.data['entries'] = sanitized_entries
    end
    
    private
    
    # Same sanitization method as AudioEntryPage
    def sanitize_title(title)
      # Remove all non-ASCII characters first
      cleaned = title.gsub(/[^\x00-\x7F]+/, '')
      # Then parameterize
